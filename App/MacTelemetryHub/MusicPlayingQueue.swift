...
enum MusicPlayingQueue {
    ...
    static func read(currentTrackID: String?, currentTitle: String?) -> AppleMusicQueueSnapshot? {
        guard let parsed = cache.load() else { return nil }
    ...
    private final class Cache: @unchecked Sendable {
        ...
        func load() -> Parsed? {
            ...
            lock.lock()
            defer { lock.unlock() }
            if let stamp, stamp == mtime, let parsed { return parsed }
            guard let fresh = Self.parse(url) else { return parsed }
            mtime = stamp
            parsed = fresh
            return fresh
        }
        ...
        private static func parse(_ url: URL) -> Parsed? {
            ...
            for segment in array(root["sega"]) ?? [] {
                ...
                for item in items {
                    ...
                    tracks.append(
                        AppleMusicQueueTrack(title: title, trackID: persistentID(spec?["tID"]))
                    )
                }
            }
